  ...
  return (
    <div className="p-4 md:p-8 h-screen flex flex-col bg-gradient-to-br from-gray-50 to-white dark:from-slate-900 dark:to-slate-800 transition-colors">
      ...
      <header className="mb-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-sm md:text-base text-gray-600 dark:text-gray-400 mt-1">Asisten AI yang responsif dan fleksibel siap ngobrol denganmu</p>
          </div>
          {messages.length > 0 && (
            <button
              onClick={clearConversation}
              className="px-4 py-2 text-sm font-medium bg-gray-200 dark:bg-slate-800 hover:bg-gray-300 dark:hover:bg-slate-700 text-gray-700 dark:text-gray-300 rounded-full transition-colors"
            >
              Mulai Baru
            </button>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0">
        {/* Chat Area */}
        <div className="lg:col-span-2 flex flex-col bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-md overflow-hidden transition-colors">
          {/* Messages Container */}
          <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 bg-gradient-to-b from-white to-gray-50 dark:from-slate-900 dark:to-slate-800">
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center px-6">
                <div className="w-20 h-20 bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-500/10 dark:to-emerald-500/20 rounded-full flex items-center justify-center mb-4 shadow-sm">
                  <Bot className="w-10 h-10 text-emerald-500 dark:text-emerald-400" />
                </div>
                <h3 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white mb-2">Halo! 👋</h3>
                <p className="text-sm md:text-base text-gray-600 dark:text-gray-400 max-w-sm mb-4">Aku adalah asisten AI Madiun yang bisa diajak ngobrol santai dan responsif. Tanyakan apapun tentang kota, lokasi, atau topik lainnya!</p>
                <div className="text-xs text-gray-500 dark:text-gray-400 space-y-2">
                  <p>Contoh pertanyaan:</p>
                  <div className="flex flex-wrap gap-2 justify-center">
                    <span className="px-3 py-1 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 rounded-full">\"Rumah sakit apa di Madiun?\"</span>
                    <span className="px-3 py-1 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 rounded-full">\"Kuliner apa yang enak?\"</span>
                    <span className="px-3 py-1 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 rounded-full">\"Cerita tentang Madiun yuk\"</span>
                  </div>
                </div>
              </div>
            ) : (
              <>
                {messages.map((message) => (
                  ...
                  >
                    {message.role === 'assistant' && (
                      <div className="w-7 h-7 md:w-8 md:h-8 bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-500/10 dark:to-emerald-500/20 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm">
                        <Bot className="w-4 h-4 md:w-5 md:h-5 text-emerald-600 dark:text-emerald-400" />
                      </div>
                    )}
                    <div
                      className={`max-w-[85%] md:max-w-md px-3 md:px-4 py-2 md:py-3 rounded-2xl text-sm md:text-base leading-relaxed ${
                        message.role === 'user'
                          ...
                          : 'bg-gray-100 text-gray-900 dark:bg-slate-800 dark:text-gray-200 rounded-bl-sm shadow-sm hover:shadow-md transition-shadow'
                      }`}
                    >
                      {message.role === 'assistant' ? (
                ...
                {loading && (
                  <div className="flex gap-2 md:gap-3 animate-fadeIn">
                    <div className="w-7 h-7 md:w-8 md:h-8 bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-500/10 dark:to-emerald-500/20 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm">
                      <Loader2 className="w-4 h-4 md:w-5 md:h-5 text-emerald-600 dark:text-emerald-400 animate-spin" />
                    </div>
                    <div className="max-w-xs md:max-w-md px-4 py-3 rounded-2xl bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-gray-400 rounded-bl-sm shadow-sm">
          ...
          {/* Input Area */}
          <div className="border-t border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 md:p-4 transition-colors">
            <form onSubmit={handleSearch} className="flex gap-2 md:gap-3">
              <input
                ...
                className="flex-1 px-4 py-2 md:py-3 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-full text-sm md:text-base text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white dark:focus:bg-slate-900 transition-all disabled:bg-gray-100 dark:disabled:bg-slate-800 disabled:cursor-not-allowed"
                disabled={loading}
                autoFocus
              />
              <button
        ...
        {/* Sidebar: Places */}
        <div className="bg-white dark:bg-slate-900 p-4 md:p-6 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-md max-h-fit sticky top-0 md:top-4 transition-colors">
          <h3 className="text-base md:text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
            <MapPin className="w-5 h-5 text-emerald-500 dark:text-emerald-400" /> Lokasi
          </h3>
          ...
          {places.length > 0 ? (
            <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
              {places.map((place, i) => (
                <a
                  ...
                  className="block p-3 rounded-xl border border-gray-200 dark:border-slate-700 hover:border-emerald-300 dark:hover:border-emerald-500/50 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 transition-all group shadow-sm hover:shadow-md"
                >
                  <h4 className="text-sm font-semibold text-gray-900 dark:text-white group-hover:text-emerald-700 dark:group-hover:text-emerald-400 mb-1 line-clamp-2">{place.title}</h4>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">Mulai ngobrol untuk melihat lokasi!</p>
            </div>
          )}
        </div>
      </div>
